    ...
    return (
        <Menu as='div' className='w-full h-full flex xl:flex-row'>
         <div className="relative flex-1">
            ...
            <Menu.Items  className="dropdown-menu  shadow-lg absolute -top-72 xl:top-[90px] left-1/2 
            xl:left-0 z-10 transform -translate-x-1/2 xl:translate-x-0 text-sm
             w-full bg-white max-w-[332px] py-6 rounder-[10px]">
             {
               hours.map((hour,index)=>{
                return (
                    <div onClick= {()=>setHour(hour)} key={index} className="cursor-pointer py-4 text-center hover:bg-gray-50">
                        {hour}
                    </div>
                );
